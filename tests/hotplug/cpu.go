...
var _ = Describe("[sig-compute]CPU Hotplug", decorators.SigCompute, decorators.SigComputeMigrations, decorators.RequiresTwoSchedulableNodes, decorators.VMLiveUpdateRolloutStrategy, Serial, func() {
	...
	FContext("A VM with cpu.maxSockets set higher than cpu.sockets", func() {
...
func getGuestVirtualCpus(vmi *v1.VirtualMachineInstance) (int, error) {
	res, err := console.RunCommandAndStoreOutput(vmi, "grep -c ^processor /proc/cpuinfo", time.Second*30)
	if err != nil {
		return -1, err
	}
