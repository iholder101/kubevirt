...
// Pointer handling appears in both the objectVal branch (for recursive merge
// into pointer-to-struct fields) and the scalar branch (for wrapping values
// into pointer-to-primitive fields like *string or *uint).
...
func setSliceField(field reflect.Value, val ref.Val) error {
	...
	ft := field.Type()
	elemType := ft.Elem()
	isElemPtr := elemType.Kind() == reflect.Pointer
