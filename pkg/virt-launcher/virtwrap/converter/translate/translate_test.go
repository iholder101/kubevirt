...
var _ = Describe("Domain translation", func() {
	...
	Context("ToLibvirtDomain", func() {
		...
		It("should convert a DomainSpec with block disk", func() {
			spec := api.NewMinimalDomainSpec("test-vm")
			spec.Devices.Disks = []api.Disk{
				{
					Type:   "block",
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(domain.Devices.Disks).To(HaveLen(1))
			Expect(domain.Devices.Disks[0].Source.Block).ToNot(BeNil())
			Expect(domain.Devices.Disks[0].Source.Block.Dev).To(Equal("/dev/sda"))

			assertDomainSpecRoundTrip(spec)
		})

		It("should convert a DomainSpec with network disk", func() {
			spec := api.NewMinimalDomainSpec("test-vm")
			spec.Devices.Disks = []api.Disk{
				{
					Type:   "network",
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(domain.Devices.Disks).To(HaveLen(1))
			Expect(domain.Devices.Disks[0].Source.Network).ToNot(BeNil())
			Expect(domain.Devices.Disks[0].Source.Network.Protocol).To(Equal("rbd"))
	...
	Context("Round-trip fidelity", func() {
		It("should round-trip a DomainSpec with ethernet interface", func() {
			spec := api.NewMinimalDomainSpec("test-vm")
			spec.Devices.Interfaces = []api.Interface{
				{
					Type: "ethernet",
