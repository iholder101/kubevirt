		...
		if newVM.Spec.LiveUpdateFeatures != nil && newVM.Spec.LiveUpdateFeatures.CPU != nil {
			...
			if oldTopology != nil && newTopology != nil {
				...
				if oldTopology.Sockets != newTopology.Sockets {
					if causeErr, allow := admitter.shouldAllowCPUHotPlug(oldVM); !allow {
						return []metav1.StatusCause{{
							Type:    metav1.CauseTypeFieldValueNotSupported,
...
func (admitter *VMsAdmitter) shouldAllowCPUHotPlug(vm *v1.VirtualMachine) (error, bool) {
	vmi, err := admitter.VirtClient.VirtualMachineInstance(vm.Namespace).Get(context.Background(), vm.Name, &metav1.GetOptions{})
	if err != nil && !errors.IsNotFound(err) {
		return nil, true
	}
	...
	for _, c := range vmi.Status.Conditions {
		if c.Type == v1.VirtualMachineInstanceVCPUChange &&
			...
			return fmt.Errorf("Cannot update CPU sockets while another CPU change is in progress"), false
		}
	}
	...
	if vmi.Status.MigrationState != nil &&
		...
		return fmt.Errorf("Cannot update CPU sockets while VMI migration is in progress"), false
	}
	return nil, true
}
