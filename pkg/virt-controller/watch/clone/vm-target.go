...
func generatePatches(source *k6tv1.VirtualMachine, cloneSpec *clonev1alpha1.VirtualMachineCloneSpec, snapshot *snapshotv1.VirtualMachineSnapshot) (patches []string) {
	...
	changeUnsnapshottedVolumeNamesPatches := generateChangeUnsnapshottedVolumeNamesPatches(source.Spec.Template.Spec.Volumes, source.Spec.DataVolumeTemplates, snapshot.Status.SnapshotVolumes.ExcludedVolumes)
...
func generateChangeUnsnapshottedVolumeNamesPatches(targetVmVolumes []k6tv1.Volume, dvTemplates []k6tv1.DataVolumeTemplateSpec, volumesExcludedFromSnapshot []string) (patches []string) {
	if targetVmVolumes == nil || volumesExcludedFromSnapshot == nil {
		return nil
	}
